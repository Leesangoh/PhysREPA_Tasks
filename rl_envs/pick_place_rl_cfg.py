...
"""Pick-and-Place RL environment configuration for Franka with joint position control.

Based on Isaac Lab's LiftEnvCfg with modifications for pick-and-place:
- Target position on the table surface (not in the air)
- Place accuracy reward
...
##
# Continuous reward replacements for binary Isaac Lab functions
##


def continuous_object_is_lifted(
    env, scale: float = 0.1, table_height: float = 0.03,
    object_cfg: SceneEntityCfg = SceneEntityCfg("object"),
) -> torch.Tensor:
    """Continuous lift reward proportional to height above table."""
    obj: RigidObject = env.scene[object_cfg.name]
    height_above = torch.clamp(obj.data.root_pos_w[:, 2] - table_height, min=0.0)
    return torch.tanh(height_above / scale)


def continuous_object_goal_distance(
    env, std: float, command_name: str,
    robot_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    object_cfg: SceneEntityCfg = SceneEntityCfg("object"),
) -> torch.Tensor:
    """Object-goal distance WITHOUT binary height gating. Always gives gradient."""
...
class PickPlaceSceneCfg(InteractiveSceneCfg):
    """Scene configuration for pick-and-place with Franka and a cuboid object."""
...
@configclass
class CommandsCfg:
    """Command terms for pick-and-place: target position on the table."""
...
class RewardsCfg:
    ...
    # Lifting the object — CONTINUOUS (not binary)
    lifting_object = RewTerm(
        func=continuous_object_is_lifted,
        params={"scale": 0.08, "table_height": 0.03},
        weight=20.0,
    )

    # Coarse tracking — NO binary height gating
    object_goal_tracking = RewTerm(
        func=continuous_object_goal_distance,
        params={"std": 0.08, "command_name": "object_pose"},
        weight=16.0,
    )

    # Fine-grained place accuracy — NO binary height gating
    ...
    # Action penalty (reduced)
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-1e-7)

    # Joint velocity penalty (reduced)
    joint_vel = RewTerm(
...
@configclass
class CurriculumCfg:
    """Curriculum terms for pick-and-place."""
...
@configclass
class PickPlaceRLEnvCfg(ManagerBasedRLEnvCfg):
...
@configclass
class PickPlaceRLEnvCfg_PLAY(PickPlaceRLEnvCfg):
    def __post_init__(self):
        super().__post_init__()
